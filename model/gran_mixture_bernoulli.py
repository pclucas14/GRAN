...
class GRANMixtureBernoulli(nn.Module):
  ...
  def _inference(self,
    ...
    # flatten batched Adj matrices to a single graph with `bs` disjoint components
    A_pad = A_pad.view(B * C * N_max, -1)

    # TODO: put this back
    # A_pad = A_pad[node_idx_feat].clone()

    if self.dimension_reduce:
      # linear projection of the node features
    ...
    if self.has_rand_feat:
      ...
      # (Lucas) at this point `att_edge_feat` has ones only on index [0, 1, self.att_edge_dim, self.add_edge_dim + 1]
      # (Lucas) ok so att_edge_feat[i] will have two non-zero entries at idx a,b s.t. ith edge is from a to b

    # GNN inference
    # N.B.: node_feat is shared by multiple subgraphs within the same batch

    # (Lucas) : how exactly is `node_idx_feat` used here ?
    # TODO: put this back
    state = node_feat[node_idx_feat]
    # state = node_feat

    node_state = self.decoder(
        state, edges, edge_feat=att_edge_feat)
  ...
  def forward(self, input_dict):
    ...
    if not is_sampling:
      ...
      test = A_pad.reshape(-1, A_pad.size(-1))

      out = F.pad(
        test, (0, 0, 1, 0), 'constant', value=0.0)  # (BCN_max + 1) X N_max
      out  = out[node_idx_feat]
...
def mixture_bernoulli_loss(label, log_theta, log_alpha, adj_loss_func,
  ...
  """ equivalent to
  adj_loss = adj_loss_func(log_theta, label.view(-1, 1).expand(-1, 20)) --> E, 20 tensor
  """
  ...
  # aggregate w.r.t nodes being generated at the same time (per masked row if B == 1)
